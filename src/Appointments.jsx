...
export default function Appointments({ client }) {
  ...
  function load() {
    supabase.from('appointments')
      .select('id, appt_date, start_min, duration_min, status, workers(name), appointment_services(price_rsd, services(name_sr, name_en))')
  ...
  function switchTab(next) { haptic('tap'); setTab(next); setIdx(0) }
  ...
  return (
    <div className="anim-in" style={{ padding: 16 }}>
      ...
      <div className="subtabs">
        <button className={tab==='up'?'on':''} onClick={()=>switchTab('up')}>{L('Predstojeći', 'Upcoming')}</button>
        <button className={tab==='past'?'on':''} onClick={()=>switchTab('past')}>{L('Prošli', 'Past')}</button>
      </div>

      {rows === null ? <SkeletonRows count={3} /> : list.length === 0 ? (
        <div className="card"><p className="tiny">
          {tab==='up'
            ? L('Nemate zakazanih termina.', 'You have no upcoming appointments.')
            : L('Nema prošlih termina.', 'No past appointments.')}
        </p></div>
      ) : (
        <div>
          <AnimatePresence mode="wait">
            <motion.div key={tab + idx}
              ...
              <Card a={current} active={tab==='up'} onCancel={cancel} lang={lang} />
            </motion.div>
          </AnimatePresence>
          ...
          {list.length > 1 && (
            <div className="apptnav">
              <button className="ghost" disabled={idx===0}
                onClick={() => { haptic('tap'); setIdx(i => i-1) }}>‹ {L('Prethodni', 'Previous')}</button>
              <span className="tiny">{idx+1} / {list.length}</span>
              <button className="ghost" disabled={idx===list.length-1}
                onClick={() => { haptic('tap'); setIdx(i => i+1) }}>{L('Sledeći', 'Next')} ›</button>
            </div>
          )}
        </div>
      )}
...
function Card({ a, active, onCancel, lang }) {
  const L = (sr, en) => lang === 'en' ? en : sr
  const status = a.status === 'cancelled'
    ? L('Otkazano', 'Cancelled')
    : active ? L('Potvrđeno', 'Confirmed') : L('Završeno', 'Completed')

  return (
    <div className="card" style={{ opacity: active ? 1 : 0.7, borderLeft: active ? '2px solid var(--rouge)' : '1px solid var(--line)' }}>
      <div className="name">{a.appointment_services.map(x => sname(x.services, lang)).join(' + ')}</div>
      <div className="tiny">{a.workers.name} · {niceDate(a.appt_date, lang)} · {fmt(a.start_min)}–{fmt(a.start_min+a.duration_min)}</div>
      <div style={{ marginTop: 8 }}>
        <span className={'tag' + (active ? ' g' : ' n')}>{status}</span>
      </div>
      {active && <button className="ghost" style={{ marginTop: 10 }} onClick={() => onCancel(a.id)}>{L('Otkaži termin', 'Cancel appointment')}</button>}
    </div>
  )
}